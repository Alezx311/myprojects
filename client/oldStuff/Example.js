...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function normalizeFace(positions) {
    ...
    for (var i = 0; i < positions.length; i++) {
      var p = [(positions[i][0] - size.minX) / size.deltaX, (positions[i][1] - size.minY) / size.deltaY]
  ...
  function ToggleInstrument(p1, p2, minX, minY, options) {
    ...
    this.render = function () {
      ...
      delta.textContent =
        this.p1 + ':' + this.p2 + ' ' + parseInt(this.deltaX * 100) + ',' + parseInt(this.deltaY * 100)
      if (this.active) {
  ...
  face.smile = new ToggleInstrument('leftMouth', 'rightMouth', 1.4, -1, { note: 2, display_both: 1 })
  ...
  face.eyebrowLeft = new ToggleInstrument('pupilLeft', 'eyebrowLeft', -1, 0.6, { note: 5 })
  face.eyebrowRight = new ToggleInstrument('pupilRight', 'eyebrowRight', -1, 0.6, { note: 6 })
  ...
  for (var f in face) {
    face[f].setContainer(instruments)
  }
  ...
  setInterval(function () {
    ...
    if (Math.abs(curSize.deltaX - avg[0]) > STABLE_THRESHOLD) {
      return
    }
    if (Math.abs(curSize.deltaY - avg[1]) > STABLE_THRESHOLD) {
      return
    }
